...
class QuestionFrame: 
    ...
    def actual_question(self):
        ...
        self.last_number = 5 #This value is for testing for now. Record the number of questions the user has to do. Works with the backend
        ...
        self.questions = {1:"What is the square root of 4", 2:"What is the square of 4",3:"What is the square root of 16",4:"What is the square of 16",5:"What is the sum of 1 and 2"}
        ...
        #This values are for testing purposes for now
        #Getting the options for each question
        self.questions_option =  {1:"Options",2:"Options",3:"Without Options",4:"Options",5:"Without Options"} #This variables work with the backend
        self.option = self.questions_option[self.question_number]
        
        #Getting the options for all the questions that have options
        self.if_options = {1:[1,2,3,4], 2:[4,2,8,16],4:[8,4,32,256]} #Works with the backend
    ...
    def review(self):
        ...
        self.got_right_marks = round(0) #The number of marks he got. This variable works with the backend. This value is for testing for now
        self.total_marks = 10 #Total number of marks. This variable works with the backend. This value is for testing for now
...
class MenuFrame: 
    ...
    def editor_frame(self): #Contains the search, add, edit and delete
        ...
        def option_question_widget():            
            ...
            options = [] #This list stores the option choosen as the right answer for a question(if the question type is of options)
            
            #Right Answer func
            def right_answer(option): #This function is used to highlight/unhighlight and store an option as a right answer
                if option in options and len(options) == 1: #If there is an option that was picked for a right answer. 
                #This "if" block is used when the user wants to remove an option that was picked as the right answer
                    ...
                    options.remove(option) #Removing the option that was picked as the right answer
                    self.right_answer_picked = False #Remembering that a right answer wasn't picked
                elif option not in options and len(options) == 0: #If there isn't an option that was picked for a right answer
                #This "elif" block is used when the user wants to add an option to be picked as the right answer
                    ...
                    options.append(option) #Adding the option that is to be picked as the right answer
        ...
        def change_subject_details_page():
            ...
            self.minutes = "00" #This value is for testing
            self.seconds = "00" #This value is for testing
            self.time_var = tk.StringVar(value=f"{self.hours}:{self.minutes}:{self.seconds}") #This value is for testing
            self.time_number = ctk.CTkEntry(self.detail_frame, border_width=0, corner_radius=10, width=200, justify="center", textvariable=self.time_var)
            ...
            if self.valid_time_bool == True and self.valid_marks_bool == True:
                def exist(topic):
                    if topic not in self.subjects_list:
                        ...
                        ques = tk.messagebox.askyesno(title="Topic Existed", message="It is noticed that this topic already exist. Do you want to change the existing subject's title?")
                        if ques == True:
                            self.detail_cancel_func() #And backend stuff
                        elif ques == False:
                            pass
        ...
        def edit_question_info_continue_func():
            ...
            def cancel_func():
                ...
                #Remving the save button as it is useless here
                self.save_button.pack_forget()
        ...
        def treeview():
            ...
            for _ in range(0,30):
                __ = _ + 1
                self.table.insert(parent="", index=_, values=(__, "Option"))
        ...
        def delete_question_page():
            ...
            #Changing th functionality of the treeview binding
        ...
        def question_option_filled(func):
            ...
            #This elif block runs if the user doesn't fill all the required spaces
                ...
                tk.messagebox.showinfo("Empty Textbox", "Please fill in all the necessary details in the textbox provided in the window")
        ...
        #Testing Purposes
        #These are the subjects of the app. For now, they aren't the real subjects
        self.subjects_list = ["Maths","Chemistry","History","Geography","Biology","Physics","IT","PE","Economics","Sociology","Business","Photography"] 
